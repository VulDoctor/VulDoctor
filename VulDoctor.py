...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for batch in bar:
        ...
        if correct_pred:
            ...
            accuracy.append(1)
            if not os.path.exists(f'./{args.datasets}'):
                os.makedirs(f'./{args.datasets}')
            with open(f'./{args.datasets}/test.json', 'a', encoding='utf-8') as f:
                f.writelines(json.dumps({
                    "source": tokenizer.decode(input_ids[0], skip_special_tokens=False),
                    "raw_predict": correct_prediction
                }, ensure_ascii=False)+'\n')
        else:
    ...
    logger.info("Exact Match: {}".format(sum(accuracy)))
    ...
    # if not os.path.exists(f"./{args.datasets}"):
    #     os.makedirs(f"./{args.datasets}")
    # df.to_csv(f"./{args.datasets}/VulRepair_raw_preds.csv")
...
def main():
    ...
    parser.add_argument('--datasets', type=str, choices=['bigvul_cvefix', 'MegaVul'], default='bigvul_cvefix',
                        help="training epochs")
    ...
    # Training
    if args.do_train:
        train_data_whole = datasets.load_dataset("./cvefixes_bigvul", split="train")
